...
export function Sidebar({ appSubtitle, modelSelectorTitle, modelSelectorAriaLabel }: Readonly<SidebarProps>) {
    ...
    <aside className="flex w-full shrink-0 flex-col border-b border-slate-200 bg-slate-50 px-3 py-4 dark:border-slate-800 dark:bg-slate-900 md:w-[280px] md:border-b-0 md:border-r">
      <div className="mb-4 flex items-center gap-3 px-2 md:mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">{appSubtitle}</p>
        </div>
      </div>
      <div className="space-y-5">
        <ModelSelector
          ...
          ariaLabel={modelSelectorAriaLabel}
        />
      </div>
    </aside>
  )
